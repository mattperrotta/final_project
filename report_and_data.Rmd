...
```{r}
flu_demo_df %>% 
  ggplot(aes(x = edate, y = cases_by_1k)) +
  geom_line() +
...
flu_lm = function(df){
  ...
  function_fit = df %>%
  lm(cases ~ week + week_2 + week_3 + week_4 + week_5 + sin_f1 + cos_f1, data = df, na.action = na.exclude) 
...
epi_threshold = function(x){
  ...
  base_fit = flu_data %>%
    lm(a_total ~ week + week_2 + week_3 + week_4 + week_5 + sin_f1 + cos_f1, data = ., na.action = na.exclude)
