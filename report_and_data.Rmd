...
```{r}
tidy_practice = tidy_flu_3 %>% 
  filter(fluregion == 'North America',
         year == '2008')

epi_threshold_na = tidy_practice %>% 
  glm(log(cases_by_1k) ~ week + cos(2*pi*(1/356)*week) + sin(2*pi*(1/356)*week), data = .)

threshold_df = data.frame(epi_pred = predict(epi_threshold_na, tidy_practice), count = tidy_practice$cases_by_1k)

  ggplot(data = tidy_practice, aes(x = week, y = cases_by_1k)) +
  geom_line() +
