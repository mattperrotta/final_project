...
tidy_flu = flu_df %>% 
  ...
  group_by(fluregion, epiweek, year, week) %>% 
...
epi_threshold_na = tidy_practice %>% 
  glm(log(inf_a_prop) ~ week + cos(2*pi*(1/52)*week) + sin(2*pi*(1/52)*week), data = .)
...
flu_fit <- fluserf(tidy_practice, outc = a_total, time = week, period = 52) 
```
